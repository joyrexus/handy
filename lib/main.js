...
(function() {
  var method, name, print, run, save, view, _ref;
  ...
  view = function(file) {
    'View sample file.';    if (!file) {
      ...
      return print("Viewing " + file);
    }
  };

  run = function() {
    'Run as a command-line script.\n  ';
    var argv, file, frames;

    argv = require('optimist').alias('r', 'record').describe('r', 'Number of frames to record').argv;
    file = argv._[0];
    if (argv.record) {
      frames = parseInt(argv.record);
      return save(frames, file);
    } else {
      return view(file);
    }
  };
